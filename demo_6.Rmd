...
k = 0:numFlips
numHeads = sum(coinFlips == "H")
binomDensity_1 = tibble(k = k, p = dbinom(k, size = numFlips, prob = 0.2))

sum(binomDensity_1$p)
```
...
```{r}
arranged_probs <- binomDensity %>% arrange(p)
arranged_probs
```
...
```{r}
arranged_probs_desc <- binomDensity %>% arrange(desc(p))
arranged_probs_desc
# most likely to get 50 heads
```
...
This the distribution of flips of this coin is not centered around 50

9) Change the size of the reject region to a smaller value, what is our conclusion now?

It's found within the p < 0.05 area (in red), so yes.
...
par(mfrow=c(2,2))
plot(lm.fit)
# NOTE QQ plot - residuals are not gaussian distributed
```

```{r}
# makes an excel-like pop-up display
fix(Boston)
```
...
lstat is more useful predictor
...
res.anova
# a bigger F score is good and tells us that it explains more of the variance!
```
