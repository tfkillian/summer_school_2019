...
library("BiocFileCache", lib.loc="~/R/x86_64-pc-linux-gnu-library/3.6")
...
```{r}
dge <- readRDS("airway_dge.rds")
ds_se <- readRDS("airway_ds_se.rds")
```
...
```{r}
head(colData(vsd), 3)
#if you want the actual values, use assay()
```
...
```{r}
# will do a PCA on the genes that are varying the most
...
# edgeR and DESeq2 aren't doing different things. just pick one
```
...
```{r}
# mapIds to add additional annotations
